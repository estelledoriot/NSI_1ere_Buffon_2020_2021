...
assert sorted(diviseurs_optimise(20)) == [
    1,
    2,
    4,
    5,
...
def dichotomie(a: int, b: int, n: int) -> float:
    ...
    elle renvoie la moyenne arithmétique des valeurs xn et xn + 10**-n telles que 𝛼 ∈ [𝑥^𝑛 ; 𝑥^𝑛 + 10^−𝑛]
    """
    debut = a
    fin = b
...
assert abs(dichotomie(1, 2, 6) - 1.41421365737915) < 10**-6, "/!\\ dichotomie()"
...
def binom(n: int, k: int) -> int:
    ...
    while j <= n:
        ...
        while (
            i <= k and i <= j
        ):  # on arrête de remplir la ligne suivante lorsqu'on arrive au bout (i = j) ou lorsqu'on arrive à k
            if i == j:
...
def galton(p: int, n: int) -> list:
    ...
    current = [
        n
    ]  # liste qui contient les billes à l'étape en cours. Initialement, toutes les billes vont heurter le seul clou de l'étape 0
    for i in range(1, p + 1):  # i est l'indice de l'étape
        next = [0] * (
            i + 1
        )  # liste contenant la répartition des billes à l'étape suivante
        for j in range(len(current)):  # parcourt de la liste current
            for k in range(
                current[j]
            ):  # pour chaque bille, on détermine sa position à l'étape suivante
                p = randint(0, 1)
                if p == 0:
                    next[j] = next[j] + 1
                else:

...
def hexa_valide(ch):
    for l in ch:
        ptc = ord(l)
        if not (ord("0") <= ptc and ptc <= ord("9")) \
        and not (ord("A") <= ptc and ptc <= ord("F")):
            return False
    return True
...
print(nom_prenom(["Alan Turing", "Ada Lovelace", "Donald Knuth", \
"Claude Shannon", "Grace Hopper"]))
...
print("Nombre d'occurrences de a dans alea jacta est:", nb_occurrences("alea jacta est","a"))
...
print("Lettre la plus fréquente:", lettre_plus_frequente("Le francais est une langue romane de la famille des langues indo europeennes"))

...
def elements_bloc(sudoku, i, j):
    ...
    for k in range(
        3 * (i // 3), 3 * (i // 3) + 3
    ):  # on parcourt les lignes du bloc de la case i,j
        for l in range(
            3 * (j // 3), 3 * (j // 3) + 3
...
def possibles(sudoku, i, j):
    ...
    if (
        sudoku[i][j] != 0
    ):  # si la case est déjà remplie, il n'y a pas d'éléments possibles
        return []
    else:
        liste = []
        for a in range(1, 10):
            ...
            if possible:
                for k in range(
                    ...
                    for l in range(
                        ...
                        if (
                            sudoku[k][l] == a
                        ):  # si a est dans le bloc de i,j, il n'est pas possible pour la case i,j
                            possible = False
            if possible:
                liste.append(a)
        return liste
    ...
    for i in range(9):  # on parcourt les lignes
        ...
        for a in manquants:  # on regarde chaque élément manquant dans la ligne i
            ...
            for j in range(9):  # on parcourt les cases de la ligne i
                for e in possibles(
                    sudoku, i, j
                ):  # on parcourt tous les éléments possibles de la case i,j
                    if e == a:
                        indice = j
                        possibilite += 1
                        break
            if (
                possibilite == 1
            ):  # s'il n'y a qu'une seule case possible dans la ligne i
                sudoku[i][indice] = a
                changement = True
    return changement
...
def resoud_colonnes(sudoku):
    ...
    for j in range(9):  # on parcourt les colonnes
        ...
        for a in manquants:  # on regarde chaque élément manquant dans la colonne j
            ...
            if (
                possibilite == 1
            ):  # s'il n'y a qu'une seule case possible dans la colonne j
                sudoku[indice][j] = a
                changement = True
    return changement
...
def resolution_simple(sudoku):
    ...
    while changement:
        ...
        changement = (
            changement_cases
            or changement_lignes
            or changement_colonnes
            or changement_blocs
        )
    return copie
